...
public class Kit {
    ...
    init(contractAddress: Address, ethereumKit: WEthereumKit.Kit, transactionManager: ITransactionManager, balanceManager: IBalanceManager, allowanceManager: AllowanceManager, state: KitState = KitState()) {
    ...
    private func onUpdateSyncState(syncState: WEthereumKit.SyncState) {
        switch syncState {
        case .synced:
...
extension Kit {
    ...
    public static func instance(ethereumKit: WEthereumKit.Kit, contractAddress: Address) throws -> Kit {
    ...
    public static func addTransactionSyncer(to evmKit: WEthereumKit.Kit) {
        let syncer = Erc20TransactionSyncer(provider: evmKit.transactionProvider, evmKit: evmKit)
        evmKit.add(transactionSyncer: syncer)
    }

    public static func addDecorators(to evmKit: WEthereumKit.Kit) {
